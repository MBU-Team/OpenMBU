...
// We are using WIN32_LEAN_AND_MEAN and mmreg.h requires things from windows.h, so we need to include it first
#include <windows.h>
#include <mmreg.h>
   ...
   : SFXProvider( "FMod" )
{
   ...
   mFMod.dllRef = OsLoadLibrary( "fmodex.dll" );

   if(!mFMod.dllRef)
   {
      Con::errorf("SFXFMODProvider - Could not locate the FMod.dll!");
      return;
   }
