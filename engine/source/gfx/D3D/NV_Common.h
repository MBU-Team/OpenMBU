...
// delete a pointer allocated with new <type>[num], and set the pointer to NULL.
	...
	{																	\
		FMsg("SAFE_DELETE_ARRAY: %35s = 0x%8.8X\n", #p, p );			\
		if( p != NULL )													\
		{																\
			delete [] (p);												\
			p = NULL;													\
	...
	{															\
		FMsg("SAFE_ARRAY_DELETE: %35s = 0x%8.8X\n", #p, p );	\
		if( p != NULL )											\
		{														\
			delete [] (p);										\
			p = NULL;											\
