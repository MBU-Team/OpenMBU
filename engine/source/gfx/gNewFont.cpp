...
void GFont::addSheet()
{
    ...
    GFXTexHandle handle = GFXTexHandle(bitmap, &GFXFontTextureProfile, true);
...
bool GFont::read(Stream& io_rStream)
{
    ...
    for (i = 0; i < numSheets; i++)
    {
        ...
        char buf[30];
        dSprintf(buf, sizeof(buf), "newfont_%d", smSheetIdCount++);
        GFXTexHandle handle = GFXTexHandle(bmp, &GFXFontTextureProfile, true); //GFXTexHandle(buf, bmp);
...
void GFont::importStrip(const char* fileName, U32 padding, U32 kerning)
{
    ...
    for (S32 i = 0; i < sheetSizes.size(); i++)
    {
        ...
        mTextureSheets[i].getBitmap()->convertToBGRx(); // Diddle it back to RGB. This is awful.
    }
