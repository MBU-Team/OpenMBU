...
static void processNode(GuiTreeViewCtrl* tree, domNode* node, S32 parentID, SceneStats& stats)
{
   ...
   for (int i = 0; i < node->getContents().getCount(); i++)
   {
      ...
      {
         stats.numMeshes++;
         tree->insertItem(nodeID, _GetNameOrId(node), "mesh", "", 0, 0);
...
// Collect scene information from a COLLADA file and store it in a GuiTreeView
// control.
ConsoleFunction(enumColladaForImport, bool, 3, 3, "(string shapePath, GuiTreeViewCtrl ctrl)")
{
