...
void GuiTreeViewCtrl::buildItem(Item* item, U32 tabLevel, bool bForceFullUpdate)
{
    ...
    if (item->isExpanded() || bForceFullUpdate)
    {
        ...
        while (child)
        {
            ...
            buildItem(tmp, tabLevel + 1, bForceFullUpdate);
        }
    }
}
...
void GuiTreeViewCtrl::onRender(Point2I offset, const RectI& updateRect)
{
    ...
    if (mDragMidPoint == AbovemDragMidPoint)
    {
        ...
        GFX->drawLine(0 + offset.x, tempY, 250 + offset.x, tempY, greyLine);
        GFX->draw2DSquare(squarePt, 6, 90);
    ...
    if (mDragMidPoint == BelowmDragMidPoint)
    {
        ...
        squarePt.y = tempY2;
        squarePt.x = 125 + offset.x;
        GFX->drawLine(0 + offset.x, tempY2, 250 + offset.x, tempY2, greyLine);
        GFX->draw2DSquare(squarePt, 6, 90);
...
void GuiTreeViewCtrl::onRenderCell(Point2I offset, Point2I cell, bool, bool)
{
    ...
    if (mFlags.test(ShowTreeLines))
    {
        ...
        for (S32 i = item->mTabLevel; (parent && i > 0); i--)
        {
            ...
            if (parent->mNext)
                GFX->drawBitmapSR(mProfile->mTextureObject, drawRect.point, mProfile->mBitmapArrayRects[BmpLine]);
    ...
    if (item->isParent() && item->mState.test(Item::MouseOverBmp))
        GFX->drawBitmapSR(mProfile->mTextureObject, drawRect.point, mProfile->mBitmapArrayRects[BmpGlow]);
    ...
    if (item->isInspectorData())
    {
        ...
        {
            GFX->drawBitmapSR(mProfile->mTextureObject, drawRect.point, mProfile->mBitmapArrayRects[bitmap]);
        ...
        if (item->getObject() && item->getObject()->isLocked())
        {
            if (mIconTable[icon])
            {
                ...
                GFX->drawBitmap(mIconTable[icon], drawRect.point);
            }
        }
        ...
        if (item->getObject() && item->getObject()->isHidden())
        {
            if (mIconTable[icon2])
            {
                ...
                GFX->drawBitmap(mIconTable[icon2], drawRect.point);
            }
        }
        ...
        if (item->isParent())
        {
            if (item->getObject() && !dStrcmp(item->getObject()->getClassName(), "SimGroup"))
            {
                if (item->isExpanded())
        ...
        {
            //		This just makes no sense - you should call setInstantGRoup *only* when you change gthe instant group@!@@
            //         if (item->isInspectorData())
        ...
        if (item->mIcon)
        {
            if (mIconTable[item->mIcon])
            {
                ...
                GFX->drawBitmap(mIconTable[item->mIcon], drawRect.point);
    ...
    if (item->mState.test(Item::Selected) && mTexSelected)
        ...
        GFX->drawBitmapStretch(mTexRollover, drawRect);
    ...
    {
        GFX->drawRectFill(drawRect, mProfile->mFillColorHL);
    }
    else if (item->mState.test(Item::MouseOverText))
    {
        GFX->drawRectFill(drawRect, mProfile->mFontColorNA);
    }
...
ConsoleMethod(GuiTreeViewCtrl, getSelectedObject, S32, 2, 2, "")
{
    return (object->getSelectedItem());
}
