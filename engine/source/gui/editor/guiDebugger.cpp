...
void DbgFileView::onRenderCell(Point2I offset, Point2I cell, bool selected, bool)
{
    ...
    if (selected)
    {
        if (mBlockStart == -1)
        {
            GFX->drawRectFill(RectI(cellOffset.x - 2, cellOffset.y - 3,
        ...
        else if (mBlockStart >= 0 && mBlockEnd > mBlockStart && mBlockEnd <= S32(dStrlen(mFileView[cell.y].text) + 1))
        {
            ...
            //draw the hilite
            GFX->drawRectFill(RectI(cellOffset.x + startPos, cellOffset.y - 3, endPos - startPos + 2, mCellSize.y + 6), mProfile->mFillColorHL);
        }
    }

    //draw the line of text
    GFX->setBitmapModulation(mFileView[cell.y].breakOnLine ? mProfile->mFontColorHL : mProfile->mFontColor);
    GFX->drawText(mFont, cellOffset, mFileView[cell.y].text);
}
