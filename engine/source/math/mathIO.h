...
//---------------------
//---------------------
// Fast memstream inlines used below:
inline bool mathRead(MemStream& stream, Point2I* p)
{
    stream.read(&p->x);
    return stream.read(&p->y);
}

inline bool mathRead(MemStream& stream, Point3I* p)
{
    stream.read(&p->x);
    stream.read(&p->y);
    return stream.read(&p->z);
}

inline bool mathRead(MemStream& stream, Point2F* p)
{
    stream.read(&p->x);
    return stream.read(&p->y);
}

inline bool mathRead(MemStream& stream, Point3F* p)
{
    stream.read(&p->x);
    stream.read(&p->y);
    return stream.read(&p->z);
...
inline bool mathRead(MemStream& stream, Point4F* p)
{
    stream.read(&p->x);
    stream.read(&p->y);
    stream.read(&p->z);
    return stream.read(&p->w);
}

inline bool mathRead(MemStream& stream, Point3D* p)
{
    stream.read(&p->x);
    stream.read(&p->y);
    return stream.read(&p->z);
...
inline bool mathRead(MemStream& stream, PlaneF* p)
{
    stream.read(&p->x);
    stream.read(&p->y);
    stream.read(&p->z);
    return stream.read(&p->d);
}

inline bool mathRead(MemStream& stream, Box3F* b)
{
    mathRead(stream, &b->min);
    return mathRead(stream, &b->max);
}

inline bool mathRead(MemStream& stream, SphereF* s)
{
    mathRead(stream, &s->center);
    return stream.read(&s->radius);
}

inline bool mathRead(MemStream& stream, RectI* r)
{
    mathRead(stream, &r->point);
    return mathRead(stream, &r->extent);
}

inline bool mathRead(MemStream& stream, RectF* r)
{
    mathRead(stream, &r->point);
    return mathRead(stream, &r->extent);
}

inline bool mathRead(MemStream& stream, MatrixF* m)
{
    F32* pm = *m;
    for (U32 i = 0; i < 15; i++)
        stream.read(&pm[i]);
    return stream.read(&pm[15]);
}

inline bool mathRead(MemStream& stream, QuatF* q)
{
    stream.read(&q->x);
    stream.read(&q->y);
    stream.read(&q->z);
    return stream.read(&q->w);
}
//------------------------------------------------------------------------------
//-------------------------------------- WRITING
//
