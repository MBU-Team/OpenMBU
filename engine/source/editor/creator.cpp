...
void CreatorTree::onRenderCell(Point2I offset, Point2I cell, bool, bool) {
    ...
    if (node->isGroup()) { // If we need a box...
        GFX->drawRectFill(boxStart, boxEnd, mProfile->mFillColor); // Box background
        GFX->drawRect(boxStart, boxEnd, mProfile->mFontColor);     // Border

        // Cross line
        GFX->drawLine(boxStart.x + 2, boxStart.y + 4, boxStart.x + 7, boxStart.y + 4, mProfile->mFontColor);

        if (!node->isExpanded()) // If it's a [+] draw down line
            GFX->drawLine(boxStart.x + 4, boxStart.y + 2, boxStart.x + 4, boxStart.y + 7, mProfile->mFontColor);
    }
    else {
        // Draw horizontal line
        GFX->drawLine(boxStart.x + 4, boxStart.y + 4, boxStart.x + 9, boxStart.y + 4, mProfile->mFontColor);

        if (!node->isLast()) // If it's a continuing one, draw a long down line
            GFX->drawLine(boxStart.x + 4, boxStart.y - 6, boxStart.x + 4, boxStart.y + 10, mProfile->mFontColor);
        else  // Otherwise, just a small one
            GFX->drawLine(boxStart.x + 4, boxStart.y - 2, boxStart.x + 4, boxStart.y + 4, mProfile->mFontColor);
    }
    ...
    while (!parent->isRoot()) {
        if (!parent->isLast()) {
            GFX->drawLine(cellOffset.x + (parent->mTab * mTabSize) + 6,
                cellOffset.y - 2,
