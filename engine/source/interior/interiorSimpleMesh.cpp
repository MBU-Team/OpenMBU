...
void InteriorSimpleMesh::render(const RenderInst &copyinst, U32 interiorlmhandle, U32 instancelmhandle,
                                   LightInfo *pridynlight, LightInfo *secdynlight,
                                   GFXTexHandle *pridyntex, GFXTexHandle *secdyntex)
{
   AssertFatal((primBuff->mPrimitiveCount == packedPrimitives.size()), "Primitive mismatch");
	...
	for(S32 i=0; i<packedPrimitives.size(); i++)
	{
      ...
      RenderInst *inst = gRenderInstManager.allocInst();
      *inst = copyinst;

      inst->matInst = materialList->getMaterialInst(draw.diffuseIndex);
      if(!inst->matInst)
         inst->matInst = gRenderInstManager.getWarningMat();
      if(!inst->matInst)
         continue;
      ...
      inst->translucent = draw.alpha;

      inst->lightmap = &gInteriorLMManager.getHandle(interiorlmhandle, instancelmhandle, draw.lightMapIndex);
      inst->normLightmap = &gInteriorLMManager.getNormalHandle(interiorlmhandle, instancelmhandle, draw.lightMapIndex);

      gRenderInstManager.addInst(inst);
	}
}
...
bool InteriorSimpleMesh::write(Stream& stream) const
{
   ...
   else
   {
      ...
      {
         GFXTexHandle& handle = materialList->getMaterial(i);
...
void InteriorSimpleMesh::buildBuffers()
{
   ...
   Vector<GFXVertexPNTTBN> packedverts;
   packedverts.setSize(verts.size());

   // fill it
   for(U32 i=0; i<packedverts.size(); i++)
   {
       GFXVertexPNTTBN &v = packedverts[i];
   ...
   GFXVertexPNTTBN *rawvb = vertBuff.lock();
   dMemcpy(rawvb, packedverts.address(), packedverts.size() * sizeof(GFXVertexPNTTBN));
   vertBuff.unlock();
...
bool InteriorSimpleMesh::prepForRendering(const char* path)
{
   ...
   for(U32 i=0; i<materialList->getMaterialCount(); i++)
   {
      MatInstance *inst = materialList->getMaterialInst(i);
      if(inst)
         inst->init(sgData, (GFXVertexFlags)getGFXVertFlags(vert));
   }
