...
class Journal 
{
   ...
   static Stream *mFile;
   static enum Mode {
      StopState, PlayState, RecordState
   } _State;
...
public:
   ...
   static void DeclareFunction(T obj, U method, bool destroy = false) {
      // Kill ourselves off?
      if(destroy)
      {
      ...
      // Otherwise, registration as normal.
   ...
   #define Method(Func,Arg1,Arg2) \
      static typename ReturnType<Func>::ValueType Result Arg1 { \
         ...
         AssertFatal(!_Dispatching,"Journal: Cannot journal during dispatch"); \
         if (_State == PlayState) { \
            _start(); \
