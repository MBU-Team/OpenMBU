...
long OggVorbisFile::_get_prev_page(ogg_page *og){
  ...
  while(offset==-1){
    ...
    while(vf->offset<begin+CHUNKSIZE){
      ret=_get_next_page(og,begin+CHUNKSIZE-vf->offset);
...
long OggVorbisFile::ov_bitrate(int i){
  ...
  if(i<0){
    ...
    return((long)floor( (bits/ov_time_total(-1)) + 0.5f ));
  }else{
    if(vf->seekable){
      /* return the actual bitrate */
      return((long)floor( ((vf->offsets[i+1]-vf->dataoffsets[i])*8/ov_time_total(i)) + 0.5f ));
    }else{
...
long OggVorbisFile::ov_bitrate_instant(){
  ...
  vf->bittrack=0.f;
  vf->samptrack=0.f;
  return(long(vf->bittrack/vf->samptrack*vf->vi[link].rate+.5));
}
...
int OggVorbisFile::ov_raw_seek(long pos)
{
  ...
  */

  {
    ogg_page og;
    ogg_packet op;
    int lastblock=0, thisblock=0, accblock=0;
    int eosflag=0;
...
int OggVorbisFile::ov_pcm_seek_page(ogg_int64_t pos){
  ...
  /* new search algorithm by HB (Nicholas Vinen) */
  {
	...
	if(bisect<=begin)
      ...
      while(begin<end){
	ret=_get_next_page(&og,end-bisect);
