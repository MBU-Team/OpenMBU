...
bool ActionMap::processBind(const U32 argc, const char** argv)
{
...
#ifndef TORQUE_ALLOW_MULTIPLE_ACTIONMAP_BINDS
    ...
    // JMQ: the above was a tribes2-era decision.  Not sure what the
    // reasons were.  I've found it to be fairly inconvenient.
    if (pFnName[0])
    {
        U32 devMapIndex, nodeIndex;
...
bool ActionMap::processAction(const InputEvent* pEvent)
{
    ...
    if (pEvent->action == SI_MAKE)
    {
        ...
        if (pNode == NULL)
        {
            ...
            if (pEvent->modifier != 0) pNode = findNode(pEvent->deviceType, pEvent->deviceInst, 0, pEvent->objInst);
        ...
        if (pNode->flags & Node::Ranged)
        {
            ...
            if (pNode->flags & Node::Inverted) value *= -1.0f;
        }
        else
        {
            if (pNode->flags & Node::Inverted) value = 1.0f - value;
        }

        if (pNode->flags & Node::HasScale) value *= pNode->scaleFactor;
        ...
        {
            // it's a bind command
            if (pNode->makeConsoleCommand) Con::evaluate(pNode->makeConsoleCommand);
        }
    ...
    else if (pEvent->action == SI_MOVE)
    {
        if (pEvent->deviceType == MouseDeviceType)
        {
            ...
            // "Do nothing" bind:
            if (!pNode->consoleFunction[0]) return(true);
...
//------------------------------------------------------------------------------
ConsoleFunction(getCurrentActionMap, const char*, 1, 1, "getCurrentActionMap()")
{
    SimSet* pActionMapSet = Sim::getActiveActionMapSet();

    SimObject* actionMap = pActionMapSet->last();

    char* retBuf = Con::getReturnBuffer(128);
    dSprintf(retBuf, 128, "%d", actionMap->getId());
    return retBuf;
