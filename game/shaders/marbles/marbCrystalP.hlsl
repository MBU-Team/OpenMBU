...
)
{
   ...
   eyeVec = mul(objRotMatrix, eyeVec);
   float3 reflectVec = reflect(-IN.cubemapEye, norm); //2.0 * dot( IN.cubemapEye, norm ) * norm - IN.cubemapEye * dot( norm, norm );
   float3 refractVec = refract(IN.cubemapEye, norm, 1.8);

   // Sample the cubemap.
   float4 refractColor = texCUBE(cubeMap, refractVec);
   ...
   OUT.color = lerp(texColor, float4(1,1,1,1), 0.25) *  lerp(refractColor,
                    finalReflectColor,
                    fresnelTerm) /* * diffuse +
               specular * bumpColor.a */;
   //OUT.color.rgba = fresnelTerm;
   return OUT;
}
