...
U32
SheetManager::enterLightMap(const GBitmap* in_pData)
{
   numPixels += in_pData->getWidth() * in_pData->getHeight();
   ...
   if (m_currX + in_pData->getWidth() >= csm_sheetSize) {
      // Carriage return...
      m_currX    = 0;
      m_currY    = m_lowestY;
   }

   if (m_currY + in_pData->getHeight() >= csm_sheetSize) {
      // new sheet needed
      setupNewSheet();
   }
   ...
   m_lightMaps.last().width   = in_pData->getWidth();
   m_lightMaps.last().height  = in_pData->getHeight();

   // And place the lightmap...
   //
   AssertFatal(in_pData->bytesPerPixel == m_sheets[m_currSheet].pData->bytesPerPixel, "Um, bad mismatch of bitmap types...");

   for (U32 y = 0; y < in_pData->getHeight(); y++) {
      const U8* pSrc = in_pData->getAddress(0, y);
      U8* pDst       = m_sheets[m_currSheet].pData->getAddress(m_currX, (m_currY + y));

      dMemcpy(pDst, pSrc, in_pData->getWidth() * in_pData->bytesPerPixel);
   }

   m_currX += in_pData->getWidth();
   if (m_currY + in_pData->getHeight() > m_lowestY)
      m_lowestY = m_currY + in_pData->getHeight();
