...
#include "max2dtsExporter/NvTriStripObjects.h"
...
NvEdgeInfo * NvStripifier::FindEdgeInfo(NvEdgeInfoVec &edgeInfos, int v0, int v1){
   ...
   while (infoIter != NULL){
      if (infoIter->m_v0 == v0){
         ...
         AssertFatal(infoIter->m_v1 == v0,"Strip error");
         if (infoIter->m_v0 == v1)
            return infoIter;
         else
...
NvFaceInfo * NvStripifier::FindOtherFace(NvEdgeInfoVec &edgeInfos, int v0, int v1, NvFaceInfo *faceInfo)
{
   ...
   AssertFatal(edgeInfo != NULL,"Strip error");
      ...
      if(!AlreadyExists(faceInfo, faceInfos))
      {
         ...
         if (edgeInfo01 == NULL){
            ...
            if (edgeInfo01->m_face1 != NULL)
            {
               ...
               AssertFatal(0,"BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences");
            }
            else
         ...
         if (edgeInfo12 == NULL){
            ...
            {
               // TODO: not an assert, put in dump file
               AssertFatal(0,"BuildStripifyInfo: > 2 triangles on an edge... uncertain consequences\n");
            }
            else
...
inline int NvStripifier::GetNextIndex(const WordVec &indices, NvFaceInfo *face){
   ...
   int numIndices = indices.size();
   AssertFatal(numIndices >= 2,"strip error");
   ...
   if (fv0 != v0 && fv0 != v1){
      if ((fv1 != v0 && fv1 != v1) || (fv2 != v0 && fv2 != v1))
      {
         ...
         AssertFatal(0,"GetNextIndex: Triangle doesn't have all of its vertices\n");
         AssertFatal(0,"GetNextIndex: Duplicate triangle probably got us derailed\n");
      }
      return fv0;
   }
   if (fv1 != v0 && fv1 != v1){
   ...
   // shouldn't get here
         // TODO: put into dump file
   AssertFatal(0,"GetNextIndex: Duplicate triangle sent\n");
   return -1;
}
...
inline void NvStripInfo::MarkTriangle(NvFaceInfo *faceInfo){
   AssertFatal(!IsMarked(faceInfo),"strip error");
   if (IsExperiment()){
      faceInfo->m_experimentId = m_experimentId;
      faceInfo->m_testStripId  = m_stripId;
    }
   else{
      AssertFatal(faceInfo->m_stripId == -1,"strip error");
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
   ...
   dMemset(bVisitedList, 0, tempFaceList.size()*sizeof(bool));
...
//
int NvStripifier::CountRemainingTris(Vector<NvStripInfo*>::iterator iter,
                                     Vector<NvStripInfo*>::iterator  end)
{
   int count = 0;
   while (iter != end)
   {
...
inline void SWAP(S32 & a, S32 & b) { S32 tmp = a; a=b; b=tmp; }
...
bool IsCW(NvFaceInfo *faceInfo, int v0, int v1)
{
   ...
   // shouldn't get here
   AssertFatal(0,"strip error");
   return false;
}

#include "ts/tsMesh.h"

void convertStripFormat(NvStripInfo * strip, TSDrawPrimitive & draw, U32 matIndex, Vector<U16> & indices)
{
   ...
   draw.start = indices.size();
   draw.numElements = stripIndices.size();
   draw.matIndex = matIndex;
   indices.setSize(draw.start+draw.numElements);
   dMemcpy(indices.address()+draw.start,stripIndices.address(),draw.numElements*sizeof(U16));
}

void nvMakeStrips(Vector<TSDrawPrimitive> & primitives, Vector<U16> & indices, U32 matIndex, S32 cacheSize, S32 minStripLength)
{
   ...
   stripper->Stripify(indices,cacheSize,minStripLength,strips,leftoverFaces);
   delete stripper;

   // convert into ts format...
   indices.clear();
   S32 startPrim = primitives.size();
   primitives.setSize(startPrim+strips.size());
   for (i=0; i<strips.size(); i++)
      convertStripFormat(strips[i],primitives[startPrim+i],matIndex,indices);
...
void nvStripWrap(Vector<TSDrawPrimitive> & faces, Vector<U16> & indices, S32 cacheSize, S32 minStripLength)
{
   // main strip loop...
   U32 start, end, i, sz;
   Vector<TSDrawPrimitive> someStrips;
   Vector<TSDrawPrimitive> retStrips;
   Vector<U16> someIndices;
   Vector<U16> retIndices;
   for (start = 0; start<faces.size(); start=end)
   {
      ...
      for (i=start;i<end;i++)
      {
         someIndices.push_back(indices[faces[i].start + 0]);
         someIndices.push_back(indices[faces[i].start + 1]);
         someIndices.push_back(indices[faces[i].start + 2]);
      }

      U32 matIndex = faces[start].matIndex ^ (TSDrawPrimitive::Triangles|TSDrawPrimitive::Strip);
      nvMakeStrips(someStrips,someIndices,matIndex,cacheSize,minStripLength);

      // now move strips and indices into larger list
      S32 startStrips = retStrips.size();
      retStrips.setSize(startStrips+someStrips.size());
      S32 startIndices = retIndices.size();
      retIndices.setSize(startIndices+someIndices.size());
      dMemcpy(retStrips.address()+startStrips,someStrips.address(),someStrips.size()*sizeof(TSDrawPrimitive));
      dMemcpy(retIndices.address()+startIndices,someIndices.address(),someIndices.size()*sizeof(U16));
      // now adjust start of new strips
      for (i=startStrips; i<retStrips.size(); i++)
         retStrips[i].start += startIndices;
   }
   indices = retIndices;
   faces = retStrips;
}
...
/*
inline SWAP(S32 & a, S32 & b) { S32 tmp = a; a=b; b=tmp; }
...
void CreateStrips()
{
   ...
   AssertFatal(nStripCount > 0,"strip error");
   ...
   for (int i = 0; i < nStripCount; i++)
   {
      ...
      AssertFatal(nStripFaceCount > 0,"strip error");
...
void OptimizeVertices()
{
   ...
   dMemset(indexCache, -1, sizeof(int)*vertices.size());
   ...
   AssertFatal(optimizedVerts.size() == vertices.size(),"strip error");
}
*/
