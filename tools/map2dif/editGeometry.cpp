...
bool EditGeometry::parseMapFile(Tokenizer* pToker)
{
   ...
   while(pToker->advanceToken(true))
   {
	  ...
	  U32 sglightingscale = 32.0;
	  ColorI sgselfillumination = ColorI(0, 0, 0);
      sgLightingScaleEntity *sglightingscaleentity = dynamic_cast<sgLightingScaleEntity*>(pEntity);
	  if(sglightingscaleentity)
	  {
		  sglightingscale = sglightingscaleentity->sgGetLightingScale();
		  sgselfillumination = sglightingscaleentity->sgGetSelfIllumination();
	  }
	  else if(mWorldEntity)
	  {
		  sglightingscale = mWorldEntity->sgGetLightingScale();
	  }
      ...
      while(pToker->getToken()[0] == '{')
      {
		 ...
		 rBrush.sgLightingScale = sglightingscale;
		 rBrush.sgSelfIllumination = sgselfillumination;
...
EditGeometry::Entity* EditGeometry::parseEntity(Tokenizer* pToker)
{
   ...
   else if(pToker->tokenICmp(sgLightingScaleEntity::getClassName()))
...
const char* EditGeometry::insertTexture(const char* pInsert)
{
      ...
      dStricmp(pInsert, "FORCEFIELD") != 0)
   {
      ...
      // Look in the dir + wadname.  The load function will search
      // back up the path.
      dSprintf(loadBuffer,sizeof(loadBuffer),"%s%s%s",
               gWadPath, mWorldEntity->mWadPrefix, pCopy);
      pNewBitmap = loadBitmap(loadBuffer);

      //
      if(pNewBitmap)
      {
         AssertISV(pNewBitmap->getFormat() == GFXFormatR8G8B8 || pNewBitmap->getFormat() == GFXFormatR8G8B8A8,
                   avar("Bad texture format.  Only 24-bit RGB or 32-bit RGBA allowed! %s", loadBuffer));
         mTextures.push_back(pNewBitmap);
      }
      else
      {
         AssertISV(false, avar("Unable to find texture \"%s\" (.png or .jpg)", pCopy));
         GBitmap* pNewBitmap = new GBitmap(16, 16, false, GFXFormatR8G8B8);
         mTextures.push_back(pNewBitmap);
      }
   }
   else
...
F64   gPlaneNormThresh = 0.999;
F64   gPlaneDistThresh = 0.001;
...
void EditGeometry::createBrushPolys()
{
   if( mPerformExtrusion )
   {
      ...
      mMaxBound = -Point3D(1 << 30, 1 << 30, 1 << 30);   // next comment?
   }
...
void EditGeometry::createSurfaces()
{
   ...
   // Create null surfaces for the collision brushes, if there are
   //  any...
   for(U32 i = 0; i < mSpecialCollisionBrushes.size(); i++)
   {
      mSpecialCollisionBrushes[i]->selfClip();
      for(U32 j = 0; j < mSpecialCollisionBrushes[i]->mPlanes.size(); j++)
      {
         mNullSurfaces.increment();
         NullSurface& rNullSurface = mNullSurfaces.last();

         rNullSurface.planeIndex      = mSpecialCollisionBrushes[i]->mPlanes[j].planeEQIndex;
         rNullSurface.winding         = mSpecialCollisionBrushes[i]->mPlanes[j].winding;
         rNullSurface.winding.brushId = mSpecialCollisionBrushes[i]->brushId;
         rNullSurface.flags           = 0;
      }
   }

   // Create null surfaces for the vehicle collision brushes, if there are
   //  any...
   for(U32 i = 0; i < mVehicleCollisionBrushes.size(); i++)
   {
      mVehicleCollisionBrushes[i]->selfClip();
      for(U32 j = 0; j < mVehicleCollisionBrushes[i]->mPlanes.size(); j++)
      {
         mVehicleNullSurfaces.increment();
         NullSurface& rNullSurface = mVehicleNullSurfaces.last();

         rNullSurface.planeIndex      = mVehicleCollisionBrushes[i]->mPlanes[j].planeEQIndex;
         rNullSurface.winding         = mVehicleCollisionBrushes[i]->mPlanes[j].winding;
         rNullSurface.winding.brushId = mVehicleCollisionBrushes[i]->brushId;
         rNullSurface.flags           = 0;
      }
   }
...
void EditGeometry::createBrushSurfaces(const CSGBrush& brush)
{
   ...
   for(U32 i = 0; i < brush.mPlanes.size(); i++)
   {
      ...
      if(planeFinalWindings.size() != 0)
      {
         ...
         if(dStricmp(rPlane.pTextureName, "NULL")    != 0 &&
