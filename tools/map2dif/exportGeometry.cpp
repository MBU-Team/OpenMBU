...
void EditGeometry::fillInLightmapInfo(Surface& rSurface)
{
   ...
   U32 lexelcount = rSurface.lMapDimX * rSurface.lMapDimY;
   U8 *bits = rSurface.pLightDirMap->getWritableBits();

   for(U32 i=0; i<lexelcount; i++)
   {
		*bits = 127;
		bits++;
		*bits = 127;
...
bool EditGeometry::exportToRuntime(Interior* pRuntime, InteriorResource* pResource)
{
   ...
   for(U32 i=0; i<pRuntime->mLightmaps.size(); i++)
   {
	   FileStream output;
	   output.open(avar("lm_%d.png", i), FileStream::Write);
	   pRuntime->mLightmaps[i]->writePNG(output);
   }

   for(U32 i=0; i<pRuntime->mLightDirMaps.size(); i++)
   {
	   FileStream output;
	   output.open(avar("nlm_%d.png", i), FileStream::Write);
	   pRuntime->mLightDirMaps[i]->writePNG(output);
   }
